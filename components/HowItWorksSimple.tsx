              ...
              {steps.map((step, index) => (
                <div key={index} className="relative flex gap-4 sm:gap-6">
                  <div className="flex-shrink-0">
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-accent text-white flex items-center justify-center text-lg sm:text-xl font-bold shadow-lg">
                      {index + 1}
                    </div>
                  </div>
                  <div className="flex-1 bg-white rounded-xl p-4 sm:p-5 border border-accent/10 shadow-sm">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-accent">{step.icon}</span>
                      <h3 className="text-base sm:text-lg font-semibold text-primary">{step.title}</h3>
                    </div>
                    <p className="text-sm sm:text-base text-primary/70 leading-relaxed">{step.description}</p>
                  </div>
                </div>
              ))}
            </div>
